...
* **Default IP Address:** `192.168.1.241` (configurable via web interface).
* **Port Configuration:**
...
| :--- | :--- | :--- | :--- |
| `0x0B` | Send w/ Differentiation | `[1:Cmd][1:Prio][1:Len][1-8:DALI Msg][1:Param]` | **Recommended send command.** Allows distinguishing responses from spontaneous events. `Param` Bit 0=Send Twice, Bit 1=Sequence Flag. |
| `0x01` | Send Message | `[1:Cmd][1:Prio][1:Len][1-8:DALI Msg]` | Legacy send command. Discouraged as responses are not differentiated. |
...
## 3. DALI4SW Button Events

The DALI4SW button sends standard DALI-2 "Input Notification" frames (24-bit). The gateway forwards these as `Type 0x04` messages.

**Frame Format:** `YAAAAAAA IIIIIIII EEEEEEEE`
* `YAAAAAAA`: The 7-bit short address of the input device.
* `IIIIIIII`: The 8-bit instance number (0-3 for the DALI4SW, corresponding to SW1-SW4).
* `EEEEEEEE`: The 8-bit event code.
...
### 5.1. Sending a DALI Command (QUERY STATUS to Address 1)
* **DALI Frame:** `[0x03, 0x90]`
* **Foxtron Payload (Binary):** `[0x0B, 0x00, 0x10, 0x03, 0x90, 0x00]`
* **Checksum:** `0x51`
* **Full Frame:** `b'\x010B001003900051\x17'`

### 5.2. Decoding a DALI4SW Button Event
* **Received Frame (ASCII):** `0418A1E0024A`
* **Decoded Payload (Binary):** `[0x04, 0x18, 0xA1, 0xE0, 0x02]`
* **Checksum:** `0x4A` (Valid)
* **DALI Frame (24 bits):** `A1E002` -> Address `0x50` (80), Instance `0` (SW1), Event `0x02` (Short Press).

### 5.3. Query Firmware Version
...
### 5.4. DALI Type 8 (Color Temperature) Sequence
Setting a color temperature of 3000K (Tc value 333, or `0x014D`) requires a multi-step sequence:
1.  **Set DTR1 (High Byte):** Send `[0xC3, 0x01]` (Set DTR1 to `0x01`).
2.  **Set DTR0 (Low Byte):** Send `[0xA3, 0x4D]` (Set DTR0 to `0x4D`).
