...
| `0x0E` | Receive w/o Answer (Diff.) | `[1:Cmd][1:Len][1-8:DALI Msg]` | **Confirmation for a `Type 0x0B` command** that expected no DALI response. Since v0.13.1 the driver uses this echo (and the `0x0D` one) as the TX flow-control acknowledgment: the next frame is sent only after the previous one is confirmed. |
